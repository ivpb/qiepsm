...
class DatasetLoader:
    ...
    def __init__(self,task,pretrained_model_name,corpus_path,query_path,splits,batch_size,seed,hard_neg_count=None,passage_count=None,with_idf=False,idf_max_features=5000,idf_pca_components=1000,mini_batch_size=None):
        self.task = task
        self.seed = seed
        self.splits = splits
    ...
    def init(self):
        ...
        self.train_loader = DataLoader(train_dataset, batch_size=self.mini_batch_size, shuffle=True, collate_fn=collate_train,generator=self.generator)
        if len(dev_dataset)>=1:
            self.dev_loader = DataLoader(dev_dataset, batch_size=self.mini_batch_size, shuffle=True, collate_fn=collate_dev,generator=self.generator)
